...
def open_atk_calculator(parent, atk_entry_var: tk.StringVar, base_atk_var: tk.DoubleVar):
    ...
    # ---- 圣遗物槽位 ----
    slot_data = {}
    art_inner = ctk.CTkScrollableFrame(container, height=300)
    art_inner.pack(fill='both', expand=True, pady=(0,6))
    for slot in ART_SLOTS:
        row = ctk.CTkFrame(art_inner)
        row.pack(fill='x', pady=2)
        ctk.CTkLabel(row, text=slot['name'], width=80).pack(side='left', padx=4)
        for st in STATS:
            if not slot[{"main_flat":"mf","main_pct":"mp","sub_flat":"sf","sub_pct":"sp"}[st]]:
    ...
    def _silent_save():
        ...
        try:
            SAVE_ATK.write_text(json.dumps(st, ensure_ascii=False, indent=2), encoding='utf-8')
        except Exception:
            pass

    def _silent_load():
        if not SAVE_ATK.exists():
            return
        try:
    ...
    def do_calc():
        ...
        for slot in ART_SLOTS:
            ...
            for st in STATS:
                info = sd.get(st)
                if info is None or not info['check'].get():
                    continue
                try:
                    val = float(info['entry'].get() or 0)
                except ValueError:
                    result_var.set('输入错误')
                    return
                if 'flat' in st:
