...
class App:
    ...
    def _open_atk_calc(self):
        ...
        atk_win = ctk.CTkToplevel(self.root)
        atk_win.title('ATK计算器 - 常驻攻击力')
        atk_win.geometry('820x700')
        atk_win.transient(self.root)
        ...
        STAT_LABELS = {'main_flat':'主词条固定ATK','main_pct':'主词条ATK%','sub_flat':'副词条固定ATK','sub_pct':'副词条ATK%'}
        ...
        base_atk_var = tk.StringVar(value='')
        ctk.CTkEntry(base_frame, textvariable=base_atk_var, width=120).pack(side='left', padx=4)
        ...
        header_widths = [80, 140, 140, 140, 140]
        headers = ['部位','主词条固定ATK','主词条ATK%','副词条固定ATK','副词条ATK%']
        ...
        def _rebuild_art_frame():
            ...
            for slot in art_slots:
                ...
                for st in STATS:
                    ...
                    if slot['key'] not in slot_data:
                        slot_data[slot['key']] = {}
                    slot_data[slot['key']][st] = {'check':cv, 'entry':ev}
        ...
        def _silent_save():
            st = {'base_atk':base_atk_var.get(), 'weapon_secondary':wpn_var.get()}
            for k, sd in slot_data.items():
                st[k] = {}
                for sk, info in sd.items():
                    st[k][sk] = {'checked':info['check'].get(), 'value':info['entry'].get()}
        ...
        def _silent_load():
            ...
            try:
                ...
                base_atk_var.set(st.get('base_atk',''))
        ...
        def do_calc():
            try:
                base = float(base_atk_var.get() or 0)
            ...
            detail_text.insert('1.0',
                f'白值 = {base:.5f}\n总ATK% = {total_pct_all*100:.5f}%\n'
                f'  武器副词条: {wpn*100:.5f}%\n  圣遗物总计: {total_pct*100:.5f}%\n' +
                '\n'.join(lines) +
                f'\n圣遗物固定ATK = {total_flat:.5f}\n\n'
        ...
        def apply_to_main():
            if not hasattr(atk_win, '_final') or atk_win._final is None:
                ...
                return
            self.entries['atk'].set(f'{atk_win._final:.5f}')
            self.base_atk_var.set(atk_win._base)
            atk_win.destroy()
        ...
        detail_text = tk.Text(container, height=8, wrap='word', font=('Arial',11),
                               bg='#2b2b2b', fg='#dce4ee', insertbackground='white', relief='flat',
                               borderwidth=0, highlightthickness=0)
        detail_text.pack(fill='both', expand=True)
        detail_text.configure(state='disabled')

        atk_win._final = None; atk_win._base = None
        # preserve fields from main
