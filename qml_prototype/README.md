...
- UGC 截图异步识别：可点击游戏窗口自动截图或选择图片文件，并支持加载遮罩、识别结果选择、应用与撤销识别值
...
主题状态是全局设置，不与配置槽绑定，单独保存在 `.genshin_damage_calculator_global.json`：`followSystem`、`darkMode` 和 `furinaTheme`。首次升级会从当前配置槽的旧主题字段自动迁移一次。
...
每个槽会保存输入值、伤害模式、百分比输入模式、条件 ATK 加成和自动保存状态。主题设置改为全局文件保存；旧存档中的槽位主题字段会被保留，但不再参与读取。
